...
def _train(model, args):
    ...
    writer = SummaryWriter("logs/{}".format(args.modules_name))
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            if not args.MIMO:
                ...
                loss_content = criterion(pred_img[0], label_img)
                label_fft = torch.rfft(label_img, signal_ndim=2, normalized=False, onesided=False)
                pred_fft = torch.rfft(pred_img[0], signal_ndim=2, normalized=False, onesided=False)
        ...
        if epoch_idx % args.valid_freq == 0:
            psnr,ssim = _valid(model, args, epoch_idx)
            print('%03d epoch \n Average GOPRO PSNR %.2f dB' % (epoch_idx, psnr))
            writer.add_scalar('PSNR_GOPRO', psnr, epoch_idx)
            print('%03d epoch \n Average GOPRO SSIM %.2f dB' % (epoch_idx, ssim))
